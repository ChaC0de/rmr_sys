...
if (isset($_POST['search'])) {
    ...
    $sql2 = "SELECT * FROM tb_dormitory 
             INNER JOIN tb_landlord 
             ON tb_dormitory.u_ID = tb_landlord.u_ID 
             WHERE tb_dormitory.status = '1' AND
...
<div class="col-12">
    <form class="d-flex justify-content-end mt-5" action="searchDorm.php" method="POST">
      <input class="form-control me-2" type="text" name="search" placeholder="ค้นหา" aria-label="ค้นหา" style="width: 300px;">
        ...
        <?php if ($num2 == 0) { ?>
          <p class="fs-5 mt-5 bg-light text-center">ไม่มีข้อมูล</p>      
        <?php } else { ?>
   <p class="fs-5 text-end mt-3 pe-5">
    <?php echo "ผลลัพธ์ทั้งหมด <span class='text-info'>$num2</span> รายการ"; ?>
    </p>
    <div class="container">
                ...
                <div class="col">
                  <div class="card-body">
                    <p class="card-text">
                      <center>
                        <div class="row">
                          <h4 class="text-start">หอพัก : <span class="text-info"><?php echo $row2['dormName']; ?></span></h4>
                        </div>
                        <?php
                        if ($row2['dormPic'] == null) {
                          echo "<img src='../img/No_image_available.svg.webp' width='250' height='auto;' class='img-fluid'>";
                        } else {
                          $fileImage = $dir . basename($row2['dormPic']);
                          echo "<img src='$fileImage' width='400' height='400' class='img-fluid'>";
                        }
                        ?>
                      </center>
                      <div class="row">
                        <p class="text-end mt-3 px-5 text-info"><?php echo $row2['land_name']; ?></p>
                        <p class="text-end px-5">สร้าง: <span class="text-info"><?php echo $row2['createDate']; ?></span> </p>
                        <a href="dorm_detail.php?dorm_ID=<?php echo $row2['dorm_ID']; ?>" class="btn btn-sm btn-outline-info">ดูเพิ่ม</a>
                      </div>
                    </p>
